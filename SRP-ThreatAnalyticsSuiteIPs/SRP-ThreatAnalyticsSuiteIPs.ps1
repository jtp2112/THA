...
USAGE:
    ...
    PS C:\> .\SRP-ThreatAnalyticsSuiteIPs.ps1 -ConfigFile .\THAIPSuite.ini -IP 199.83.135.252 -OTX
    ...
    PS C:\> .\SRP-ThreatAnalyticsSuiteIPs.ps1 -ConfigFile .\THAIPSuite.ini -IP 199.83.135.252 -OTX -AlarmID 666

    Force a Case Creation:
    PS C:\> .\SRP-ThreatAnalyticsSuiteIPs.ps1 -ConfigFile .\THAIPSuite.ini -IP 199.83.135.252 -OTX -ForceCase
...
function VirusTotalAnalysis
{
    ...
    $virusTotal = Invoke-RestMethod -Method Get -UserAgent $userAgent -Uri https://www.virustotal.com/vtapi/v2/ip-address/report?apikey=$($VTAPIKey)`&ip=$($IP)
...
function OTXAnalysis
{
    ...
    $OTXURL = "https://otx.alienvault.com/otxapi/indicator/IPv4/"
    ...
    {
        $alienJSON = Invoke-RestMethod -Method Get -UserAgent $userAgent -Body $Body -Uri "https://otx.alienvault.com/otxapi/indicator/ip/general/$($IP)"
        if ($areSubModules)
        {
    ...
    {
        $alienJSON = Invoke-RestMethod -Method Get -UserAgent $userAgent -Body $Body -Uri "$($OTXURL)geo/$($IP)"
        if ($areSubModules)
        {
    ...
    {
        $alienJSON = Invoke-RestMethod -Method Get -UserAgent $userAgent -Body $Body -Uri "$($OTXURL)url_list/$($IP)"
        if ($areSubModules)
        {
    ...
    {
        $alienJSON = Invoke-RestMethod -Method Get -UserAgent $userAgent -Body $Body -Uri "$($OTXURL)passive_dns/$($IP)"
        if ($areSubModules)
        {
    ...
    {
        $alienJSON = Invoke-RestMethod -Method Get -UserAgent $userAgent -Body $Body -Uri "$($OTXURL)malware/$($IP)"
        if ($areSubModules)
        {
            Print-HTResult -HTResult $alienJSON -Provider "OTX IP MALWARE" -Mode $FilterMode -Filter $SubModules
            if($file)
            {
                Out2File-HTResult -HTResult $alienJSON -Provider "OTX IP MALWARE" -Mode $FilterMode -Filter $SubModules -FilePath $file
            }
        }
        else
        {
            Print-HTResult -HTResult $alienJSON -Provider "OTX IP MALWARE"
            if($file)
            {
                Out2File-HTResult -HTResult $alienJSON -Provider "OTX IP MALWARE" -FilePath $file
            }
        }
    }
    if ($Providers -contains "ALL" -or $Providers -contains "HTTP_SCANS")
    {
        $alienJSON = Invoke-RestMethod -Method Get -UserAgent $userAgent -Body $Body -Uri "$($OTXURL)http_scans/$($IP)"
        if ($areSubModules)
        {
    ...
    {
        $alienJSON = Invoke-RestMethod -Method Get -UserAgent $userAgent -Body $Body -Uri "https://otx.alienvault.com/otxapi/indicators/ip/$($IP)/comments"
        if ($areSubModules)
        {
...
function MalwaresAnalysis
{
    ...
    # End of Validation
    $malwares = Invoke-RestMethod -Method Get -UserAgent $userAgent -Body $Body -Uri https://www.malwares.com/api/v2/ip/info
    if ($areSubModules)
    {
        Print-HTResult -HTResult $malwares -Provider "MALWARES IP" -Mode $FilterMode -Filter $SubModules
        if($file)
        {
            Out2File-HTResult -HTResult $malwares -Provider "MALWARES IP" -Mode $FilterMode -Filter $SubModules -FilePath $filepath
        }
    }
    else
    {
        Print-HTResult -HTResult $malwares -Provider "MALWARES IP"
        if($file)
        {
            Out2File-HTResult -HTResult $malwares -Provider "MALWARES IP" -FilePath $filepath
        }
    }
...
if($All -or $VT)
{
    VirusTotalAnalysis -iniContent $iniContent -IP $IP -userAgent $userAgent -file $case -filepath "$($uuid.Guid)_VT" -result ([ref]$danger)
}

if($All -or $OTX)
{
    OTXAnalysis -iniContent $iniContent -IP $IP -userAgent $userAgent  -file $case -filepath "$($uuid.Guid)_OTX" -result ([ref]$danger)
}

if($All -or $MW)
{
    MalwaresAnalysis -iniContent $iniContent -IP $IP -userAgent $userAgent -file $case -filepath "$($uuid.Guid)_MW" -result ([ref]$danger)
}
...
if ($ForceCase -or ($case -and $danger))
{
    ...
    $caseName = "IP $($IP) found to be Dangerous"
    if ($danger)
    {
        $caseSummary = "IP $($IP) has been investigated in the selected Threat Hunting providers and it's been marked as dangerous or infected"
    }
    else
    {
        $caseSummary = "IP $($IP) has been requested to be investigated"
    }
